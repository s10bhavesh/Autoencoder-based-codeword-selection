...
class CODEBOOK_GENERATION:
    ...
    def codebook_generation(self):
        ...
        """
        x = [[list(i[x:x+RIS_rows]) for x in range(0, len(i), RIS_rows)] \
                for i in product(['0','180'], repeat = RIS_rows*RIS_cols)]
        codebook = np.array(x, dtype = 'int64')
       
        Configurations = tf.dtypes.cast(tf.convert_to_tensor(codebook.\
                            reshape(2**(RIS_rows*RIS_cols),RIS_elements)), tf.float64)
        rad_to_deg = m.pi/180
    ...
    def calculate_SNR(self):
        ...
        for angle in Azimuth_reflection:
            ...
            snr_value = np.multiply(self.EbNo, np.multiply(distance,(abs(tf.matmul(phases, H))**2)))
    ...
    def snr_preprocessing(self):
        ...
        for angle in Azimuth_reflection:
            df = {}
            unq_snr = pd.DataFrame({'SNR':sorted_snr[angle]['SNR']}, index=\
                                   sorted_snr[angle]["codeword"])
            unq = unq_snr[unq_snr["SNR"].duplicated(keep=False)]
            df = unq.groupby(list(unq)).apply(lambda x:set(sorted(x.index))).to_dict()
            code_snr_set[angle] = df
